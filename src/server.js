...
app.post('/api/conversations/dm', requireUser, async(req,res,next)=>{try{const {userId}=parse(z.object({userId:z.string()}),req.body);if(userId===req.user.id)return res.status(400).json({error:'Invalid recipient'});const target=await prisma.user.findUnique({where:{id:userId}});if(!target||target.disabled)return res.status(404).json({error:'User not found'});if(await isBlocked(req.user.id,userId))return res.status(403).json({error:'Cannot message this user'});const existing=await prisma.conversation.findFirst({where:{type:'DM',AND:[{members:{some:{userId:req.user.id}}},{members:{some:{userId:userId}}}]},include:{members:{include:{user:true}}}});if(existing)return res.json({conversation:{id:existing.id,members:existing.members.map(m=>safeUser(m.user))}});const c=await prisma.conversation.create({data:{type:'DM',members:{create:[{userId:req.user.id},{userId}]}}});res.json({conversation:{id:c.id,members:[safeUser(req.user),safeUser(target)]}});}catch(e){next(e);}});
