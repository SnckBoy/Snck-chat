...
async function canAccessConversation(userId, conversationId) { const m = await prisma.conversationMember.findUnique({ where: { conversationId_userId: { conversationId, userId } } }); return !!m; }
...
app.get('/api/conversations', requireUser, async(req,res,next)=>{try{const rows=await prisma.conversation.findMany({where:{members:{some:{userId:req.user.id}}},include:{members:{include:{user:true}},messages:{orderBy:{createdAt:'desc'},take:1}},orderBy:{createdAt:'desc'}});res.json({conversations:rows.map(c=>({id:c.id,type:c.type,members:c.members.map(m=>safeUser(m.user)),lastMessage:c.messages[0]?safeMessage(c.messages[0]):null}))});}catch(e){next(e);}});
