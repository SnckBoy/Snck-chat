...
else R=''; B=''; RED=''; GREEN=''; YELLOW=''; BLUE=''; CYAN=''; MAGENTA=''; WHITE=''; fi
...
install_deps(){
  step "Installing required dependencies"; require_root
  ...
  [[ "$major" =~ ^[0-9]+$ ]] || major=0
  if (( major < NODE_MAJOR )); then curl -fsSL "https://deb.nodesource.com/setup_${NODE_MAJOR}.x" | bash -; DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs; fi
  command -v node >/dev/null || die "Node.js installation failed."; command -v npm >/dev/null || die "npm installation failed."
  local nv mv; nv="$(node --version)" || die "Node.js version check failed."; mv="$(npm --version)" || die "npm version check failed."; ok "Node $nv • npm $mv"
}
...
setup_db(){
  step "Configuring PostgreSQL"; start_postgres || die "PostgreSQL is not reachable on 127.0.0.1:5432."; DBPASS="${SNCK_DB_PASSWORD:-$(openssl rand -hex 32)}"; local escaped; escaped="$(sql_escape "$DBPASS")"
...
clone_or_update(){
  ...
  if [[ -f "$APP_DIR/package.json" && -f "$APP_DIR/src/server.js" ]]; then
    ok "Using existing source: $APP_DIR"; if [[ -d "$APP_DIR/.git" ]]; then cd "$APP_DIR"; info "Checking for latest Snck Chat fixes..."; git fetch --quiet origin main && git reset --hard --quiet origin/main || warn "GitHub unavailable; continuing with existing source."; fi; return 0
  fi
...
write_env(){
  mkdir -p "$APP_DIR"; if [[ ! -f "$APP_DIR/.env" ]]; then cat > "$APP_DIR/.env" <<EOF
NODE_ENV=production
PORT=${SNCK_PORT:-3000}
DATABASE_URL=postgresql://snck:${DBPASS:-$(openssl rand -hex 32)}@127.0.0.1:5432/snck_chat
...
EOF
    ...
    load_env
    if [[ -z "${DATABASE_URL:-}" ]]; then DBPASS="${SNCK_DB_PASSWORD:-${DBPASS:-$(openssl rand -hex 32)}}"; printf '\nDATABASE_URL=postgresql://snck:%s@127.0.0.1:5432/snck_chat\n' "$DBPASS" >> "$APP_DIR/.env"; ok "DATABASE_URL added to existing .env"; fi
    grep -Eq '^PORT=' "$APP_DIR/.env" || printf 'PORT=%s\n' "${SNCK_PORT:-3000}" >> "$APP_DIR/.env"; grep -Eq '^NODE_ENV=' "$APP_DIR/.env" || printf 'NODE_ENV=production\n' >> "$APP_DIR/.env"; chmod 600 "$APP_DIR/.env"; warn "Existing .env preserved and repaired where required"
  fi
}
require_database_url(){ load_env; [[ -n "${DATABASE_URL:-}" ]] || die "DATABASE_URL is missing from $APP_DIR/.env."; }
prepare_app(){
  cd / || die "Cannot access filesystem root."; load_env; setup_db; write_env; load_env; require_database_url; cd "$APP_DIR" || die "Cannot access application directory: $APP_DIR"
  step "Installing application dependencies"; npm install --include=dev
  step "Validating Prisma schema"; npx prisma validate --schema "$APP_DIR/prisma/schema.prisma"
  step "Generating Prisma Client"; npx prisma generate --schema "$APP_DIR/prisma/schema.prisma"
  step "Synchronizing database schema"; npx prisma db push --schema "$APP_DIR/prisma/schema.prisma" --skip-generate
  if [[ -f "$APP_DIR/prisma/seed.js" ]]; then step "Initializing database"; DATABASE_URL="$DATABASE_URL" node "$APP_DIR/prisma/seed.js"; fi
  step "Running application checks"; npm run check; npm test; ok "Application checks passed"
}
write_service(){
  [[ "$WORKSPACE_MODE" == true ]] && return 0; local group; group="$(id -gn "$APP_USER")"; cat > "/etc/systemd/system/${SERVICE}.service" <<EOF
[Unit]
...
write_nginx(){
  [[ "$WORKSPACE_MODE" == true ]] && return 0; load_env; local port="${PORT:-3000}"; [[ "$port" =~ ^[0-9]+$ ]] || die "Invalid PORT in .env: $port"; (( port>=1 && port<=65535 )) || die "PORT must be between 1 and 65535."; cat > /etc/nginx/sites-available/snck-chat <<EOF
server {
...
start_service(){
  [[ "$WORKSPACE_MODE" == true ]] && return 0; step "Starting Snck Chat service"; systemctl daemon-reload; systemctl reset-failed "$SERVICE" 2>/dev/null || true; systemctl enable "$SERVICE" >/dev/null; systemctl restart "$SERVICE"
  for _ in {1..15}; do systemctl is-active --quiet "$SERVICE" && { ok "Snck Chat service is running"; return 0; }; sleep 1; done
  journalctl -u "$SERVICE" -n 100 --no-pager || true; systemctl --no-pager --full status "$SERVICE" || true; die "Snck Chat service did not start. See the service log above."
}
health_check(){ load_env; require_database_url; local port="${PORT:-3000}"; step "Running final health check"; if [[ "$WORKSPACE_MODE" != true ]]; then curl -fsS --max-time 10 "http://127.0.0.1:${port}/api/health" >/dev/null || { journalctl -u "$SERVICE" -n 100 --no-pager || true; die "Application health check failed."; }; else (cd "$APP_DIR" && node src/server.js >/tmp/snck-chat-health.log 2>&1 &); sleep 2; curl -fsS --max-time 10 "http://127.0.0.1:${port}/api/health" >/dev/null || { cat /tmp/snck-chat-health.log; die "Application health check failed."; }; fi; ok "Health check passed"; }
install_app(){ check_os; require_root; cd /; install_deps; setup_user; clone_or_update; prepare_app; if [[ "$WORKSPACE_MODE" != true ]]; then chown -R "$APP_USER:$(id -gn "$APP_USER")" "$APP_DIR"; chmod 600 "$APP_DIR/.env"; write_service; write_nginx; nginx -t; start_service; nginx -t; systemctl reload nginx; fi; health_check; }
...
main(){
  case "${1:-menu}" in install) install_app;; admin) create_admin;; update) update_app;; repair) repair_app;; uninstall) uninstall_app;; menu) menu;; *) die "Usage: $0 [install|admin|update|repair|uninstall|menu]";; esac
}
main "$@"
