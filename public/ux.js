/* Snck Chat UX enhancements: keeps the existing API/backend and adds a friend -> private DM path. */
(function(){
  const escHtml=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function findFriend(id){return Array.isArray(window.friends)?window.friends.find(x=>x.id===id):null}
  ...
  window.openFriendProfile=async function(id){
    try{
      const d=await window.api('/api/users/'+encodeURIComponent(id));
      const u=d.user, isMe=u.id===window.me?.id, friend=!!findFriend(u.id);
  ...
  window.showFriends=window.showFriends||function(){};
  const originalShowRequests=window.showRequests;
  if(originalShowRequests){
    window.showRequests=function(){return originalShowRequests.apply(this,arguments)};
  }
  /* The existing app exposes profile globally; replace it with the richer relationship-aware version. */
